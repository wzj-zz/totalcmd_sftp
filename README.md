...
The test opens both WFX sessions, temporarily replaces each profile's rules, and verifies `-L`, `-D`, and `-R` with real TCP echo traffic. Each selected profile also receives eight consecutive public HTTPS requests through its `-D` SOCKS5 listener using local `curl.exe`; every request checks that Total Commander remains alive. Coverage includes manager Toggle, Enabled/Disabled reconnect persistence, startup failure and retry, active relay shutdown, multiple independent rules, and closure of local and remote listeners after Disable. The original rules are restored in `finally`. It requires local `curl.exe`, Python 3 on the Unix remote, PowerShell on the Windows OpenSSH remote, and public HTTPS reachability. Override the endpoint when needed with `-PublicSocksUrl '<url>'`.
...
## Updating A Portable Package

To update a clean portable Total Commander ZIP with the current Release plugin package, use:

```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\update-portable-package.ps1 -ArchivePath '<Total Commander ZIP>'
```

This is different from instance deployment. The script temporarily extracts the ZIP, replaces its SFTP plugin directory, registers the router shortcuts and user commands, verifies the result, atomically replaces the original ZIP, and removes the temporary directory. It does not start the clean Total Commander instance. Close all running Total Commander instances before running it.
