...
- Standard flow when the user supplies only `<Total Commander>`: run the default preflight below, build Release, deploy the generated `SFTP\` package, then run both documented archive and tunnel smoke commands. Do not rediscover the script parameters or ask for target names when preflight passes.
...
- Deployment command, replacing `<Total Commander>` before running. It moves both panels to `C:\`, closes Total Commander cleanly, waits for it and the archive router to exit, replaces only the portable SFTP plugin directory, registers shortcuts without a dialog, then restarts Total Commander:
  ...
  ```
- Default archive smoke command, replacing `<Total Commander>` before running:
  ```powershell
...
- Before the standard full deployment flow, verify that `sftpplug.ini` contains all three default session display names and that both default WSL distributions are available through `wsl.exe` and `\\wsl.localhost\<distro>\tmp`. A standalone archive smoke requires its selected sessions and WSL targets; a standalone tunnel smoke requires only its selected Unix and Windows profiles. When a required target is unavailable, report it and stop; do not guess replacements, skip it, or modify private configuration.
...
- The tunnel smoke payload must traverse the deployed plugin listener, the profile's active libssh2 session, and a disposable endpoint. Checking only status or a listening port is insufficient. Independent `ssh.exe` commands may create endpoints and assert results, but must never replace the tunnel under test with direct `ssh -L/-R/-D` forwarding.
- Tunnel smoke matrix: data paths `-L`, `-D`, and loopback-only `-R`; lifecycle startup, enable, disable, failed, retry, active-relay shutdown, and reconnect persistence; isolation of multiple same-type rules; manager Toggle through the same load-current-state, invert, apply, and refresh action as `Ctrl+P`. Both local listeners and a real remote `-R` listener must be tested with traffic and post-disable closure.
...
- The smoke scripts create only disposable paths: `%TEMP%\SftpArchiveSmoke-*`, `%TEMP%\SftpTunnelSmoke-*`, remote `/tmp/sftpplug-archive-smoke-*`, remote `/tmp/sftpplug-tunnel-smoke-*`, Windows remote `%TEMP%\SftpTunnelSmoke-*`, and WSL UNC temp directories under detected `\\wsl.localhost\<distro>\tmp` paths or names supplied with `-WslDistros`. They always attempt to remove remote artifacts; local and WSL artifacts are retained only when `-KeepArtifacts` is supplied.
...
- Behaviors that still require UI testing in Total Commander: shortcut registration, target prompt defaults, Cancel/Enter semantics, the `Ctrl+P` profile selection and Add/Edit/Remove/Start/Stop dialog flow, tunnel status presentation, and Alt+F11/Alt+F12 UI workflows. Passing the smoke scripts proves the router-to-active-WFX-session business logic and real tunnel data paths, not the GUI interaction layer.
...
- Tunnel listeners are profile-level services owned only by the primary SSH/SFTP panel session. Background transfer or temporary cross-session connections must set `primarySession=false` and must never create or retain tunnel listeners.
- Each profile supports up to 64 independently managed rules persisted as `tunnel1` through `tunnel64`. A leading `+` means Desired Enabled and a leading `-` means Desired Disabled; runtime state is separate and must remain `Running`, `Stopped`, or `Failed`.
- `Ctrl+P` must work for disconnected profiles by editing persisted rules, and for connected profiles by applying changes immediately to the primary active session and persisting them for the next connection. Router tunnel `status`, `toggle`, and `replace` operations must acquire the primary session lease, never an arbitrary background session.
- Enabled startup failure must preserve Desired Enabled and report runtime Failed with an error so the same rule can be retried. Disable must immediately close the listener, shut down active relay sockets, and wait for tunnel workers to stop.
- Multiple rules, including multiple rules of the same type, must remain independent. Adding, removing, editing, or reordering rules must preserve unchanged running tunnel instances and restart only rules whose desired state or endpoint changed.
- Tunnels must reuse the profile's active libssh2 SSH session; do not create a separate SSH connection for forwarding. PHP Agent and LAN Pair transports do not support SSH tunnels.
...
- Use `LogProc` / `LogProcW` for Total Commander operation flow. User-visible tunnel logs must prefer `LogProcW` on x64 Total Commander and use ANSI `LogProc` only as fallback. Use `SFTP_LOG` only for internal state and errors.
